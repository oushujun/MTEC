This repository contains three historical version of curated maize TE libraries derived from the Maize TE Consortium (MTEC). I combine the three together and further clean the combined library with the following commands and curations. If you are looking for a high-quality maize TE library, look no further, `maizeTE11222019` is the one.
...
`Wessler-Bennetzen_2.fasta` was used to annotate the initial B73 genome (Schnable et al. 2009). I believe it is an earlier version of the MTEC library.
...
# remove TEs in others that are represented by consensus TEs
...
# remove redundant sequences, `history/maizeTE11212019.ori2` is the combined MTEC library
`perl bin/cleanup_nested.pl -in history/maizeTE11212019.ori2 -cov 0.98 -minlen 80 -miniden 95 -iter 2 -t 36`

# remove nonTE repeats and tandem repeats
...
# Reclassify unknown TEs
`python2 TEsorter.py history/maizeTE11212019.ori3 -p 36`

# The file `history/maizeTE11212019.ori3.rexdb.cls.tsv` contains new classifications of the library. Most of them are consistent with the old classification. What really improved are the LTR/unknown classification. Find misclassified entries:
...
# Some LTRs appear to have the same name but different classifications (RLG/RLC/RLX)
...
# Manually check misclassified sequences. These entries are put in the removal list and removed:
...
# PPP_PPO_AC185414 is changed to DTH_PPO_AC185414 manually

# Update LTR classifications
...
# convert sequence names to RepeatMasker format
...
# rename short TIR (<= 600bp) to MITE
...
# manually add back CL569186.1#subtelomere/4-12-1
